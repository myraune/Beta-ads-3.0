    ...
    <main className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <article className="streamer-surface-card streamer-beta-grid rounded-3xl p-6 md:p-8">
          ...
          <h1 className="mt-3 font-[family-name:var(--font-heading)] text-5xl font-semibold tracking-tight text-ink">Campaign marketplace</h1>
                ...
                return (
                  <article key={card.id} data-testid={`sponsorship-card-${card.campaignId}`} className="streamer-market-card overflow-hidden rounded-2xl">
                    ...
                    <div className="space-y-3 p-4">
                      ...
                      <p className="text-sm text-ink/70">{card.brand}</p>
                      <p className="text-sm leading-relaxed text-ink/75">{card.description}</p>
                      <p className="text-sm font-semibold text-ink">{card.payoutLabel}</p>
                      <div className="grid grid-cols-2 gap-2">
                        <Link
                          href={`/streamer/workspace?campaign=${card.campaignSlug}`}
                          className="streamer-ghost-btn rounded-lg px-3 py-2 text-center text-sm font-semibold text-slate-700 transition hover:border-slate-400"
                        >
                          View details
                        </Link>
                        <button
        ...
        <aside className="space-y-4">
          <div className="streamer-faq-card rounded-2xl p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-emerald-800">FAQ</p>
            <h2 className="mt-2 text-2xl font-semibold text-ink">What permissions does Beta Ads require?</h2>
            <p className="mt-2 text-sm leading-relaxed text-ink/75">
              We only request platform data needed to verify channel identity, campaign eligibility, and payout readiness.
            </p>
            <Link href="/streamer/setup" className="mt-4 inline-flex text-sm font-semibold text-emerald-800 underline">
              Read setup guidance
            </Link>
          </div>
          ...
          <div className="streamer-outline-card rounded-2xl p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm">
              <li className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${state.providerConnected ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                  {state.providerConnected ? "connected" : "pending"}
                </span>
              </li>
              <li className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                <span className="text-slate-700">OBS setup</span>
                <span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wide ${state.setupCompleted ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-700"}`}>
      ...
      <section className="space-y-3">
        <div>
          <h2 className="font-[family-name:var(--font-heading)] text-3xl font-semibold tracking-tight text-ink">Featured finished sponsorships</h2>
          <p className="text-sm text-ink/70">See completed campaign collaborations from previous flights.</p>
        </div>
          ...
          {STREAMER_FINISHED_SPONSORSHIPS.map((card) => (
            <article key={card.id} className="streamer-finished-card overflow-hidden rounded-2xl">
              ...
              <div className="p-4">
                ...
                <p className="mt-1 text-sm text-ink/70">{card.brand}</p>
                <span className="mt-3 inline-flex rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-600">
                  finished
                </span>
              </div>
            </article>
